...
module.exports = class Service {
  constructor (context, { plugins, pkg, inlineOptions, useBuiltIn } = {}) {
     /* console.log(plugins=undefined)
      console.log(pkg=undefined)
      console.log(inlineOptions=undefined)
      console.log(useBuiltIn=undefined)*/
   ...
   // console.log(inlinePkg)
    if (inlinePkg) {
      ...
      const pkg = readPkg.sync({ cwd: context })
       // console.log(pkg) //这个就是读取package.json文件
  ...
  init (mode = process.env.VUE_CLI_MODE) {
    //把这个模式赋值给 例如 mode=dev   process.env.VUE_CLI_MODE=dev
    if (this.initialized) {
      return
    }
  ...
  loadEnv (mode) {
    ...
    const logger = debug('vue:env')
      //console.log(this.context)  this.context就是命令执行的目录 /Users/apple/vue_3 
    const basePath = path.resolve(this.context, `.env${mode ? `.${mode}` : ``}`)
     //就是当前执行的那个环境的文件 /Users/apple/vue_3/.env.development
    const localPath = `${basePath}.local`
      //console.log("localPath"+localPath)   localPath =/Users/apple/vue_3/.env.local
      const load = path => {
      try {
    ...
    if (mode) {
      ...
      // as that is necessary for tests to not be affected by each other
       // shouldForceDefaultEnv   
      const shouldForceDefaultEnv = (
        process.env.VUE_CLI_TEST &&
        !process.env.VUE_CLI_TEST_TESTING_ENV
      )
        //console.log(shouldForceDefaultEnv==undifined)
  ...
  loadUserOptions () {
    ...
    const configPath = (
     ...
     // console.log(configPath) /Users/apple/vue_3/vue.config.js  
